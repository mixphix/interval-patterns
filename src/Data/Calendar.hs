...
erlangs ix e =
  let diff =
        morphism
          (morphism (\(Nominal (Picoseconds x)) -> x) :: (UTCTime -> NominalDiffTime) -> (UTCTime -> Integer))
          (flip diffUTCTime)
   in liftA2
        (/.)
        (Layers.area diff from ix e)
        (I.measuring diff ix)
...
calendar ev tf = singleton ev (Layers.singleton tf one)
...
toList (Calendar c) =
  morphism
    (morphism Layers.toList :: (ev, Layers UTCTime Natural) -> (ev, [(Interval UTCTime, Natural)]))
    (Map.assocs c)
...
totalDuration ev (Calendar c) = case c Map.!? ev of
 ...
 where
  ...
  f _ Nothing = Nothing
  f (tf, n) (Just x) = case morphism (from @_ @Integer n *.) (duration tf) of
    Nothing -> Nothing
