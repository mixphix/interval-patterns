...
import Flex.Math hiding (truncate)
...
newtype Borel x = Borel {unBorel :: Set (Interval x)}
...
instance (Ord x) => Meet (Borel x) where
  (/\) :: Borel x -> Borel x -> Borel x
  (/\) = intersection
instance (Ord x) => Join (Borel x) where
  (\/) :: Borel x -> Borel x -> Borel x
  (\/) = union
instance (Ord x) => Lowest (Borel x) where
  lowest :: Borel x
  lowest = mempty
instance (Ord x) => Highest (Borel x) where
  highest :: Borel x
  highest = whole
instance (Ord x) => Lattice (Borel x)
instance (Ord x) => Extrema (Borel x)
instance (Ord x) => Heyting (Borel x) where
  (-->) :: Borel x -> Borel x -> Borel x
  x --> y = complement x \/ y
instance (Ord x) => Boolean (Borel x)

instance (Ord x) => Addition (Borel x) (Borel x) (Borel x) where
  (+.) :: Borel x -> Borel x -> Borel x
  (+.) = symmetricDifference
instance (Ord x) => Additive (Borel x) where
  zero :: Borel x
  zero = mempty
instance (Ord x) => AdditiveAbelian (Borel x)

instance (Ord x) => Subtraction (Borel x) (Borel x) (Borel x) where
  (-.) :: Borel x -> Borel x -> Borel x
  (-.) = difference
instance (Ord x) => AdditiveGroup (Borel x) where
  negative :: Borel x -> Borel x
  negative = complement

instance (Ord x) => Multiplication (Borel x) (Borel x) (Borel x) where
  (*.) :: Borel x -> Borel x -> Borel x
  (*.) = intersection
instance (Ord x) => Multiplicative (Borel x) where
  one :: Borel x
  one = whole
instance (Ord x) => MultiplicativeAbelian (Borel x)

instance (Ord x) => Distributive (Borel x)
instance (Ord x) => Semiring (Borel x)
instance (Ord x) => Ring (Borel x)
...
toList :: (Ord x) => Borel x -> [Interval x]
toList = Set.toAscList . intervalSet
...
remove i (Borel is) = flip foldWith is \ix -> case ix I.\\ i of
  Nothing -> mempty
  Just (One j) -> borel [j]
...
intersection is (Borel js) = foldWith (`truncate` is) js

-- | Take the intersection of a list of 'Borel' sets.
intersections :: (Ord x) => [Borel x] -> Borel x
intersections = getShrink . foldWith Shrink
